...
const OUTPUT_NODE_ERROR = path.join(opts.out, "error.node.cjs");
const OUTPUT_BROWSER_ERROR = path.join(opts.out, "error.browser.mjs");

const TEMPLATE_VALIDATION_ERROR = path.join(
    __dirname,
    "../tmpl/validation.error.ejs"
);
const OUTPUT_NODE_VALIDATION_ERROR = path.join(
    opts.out,
    "validation.error.node.cjs"
);
const OUTPUT_BROWSER_VALIDATION_ERROR = path.join(
    opts.out,
    "validation.error.browser.mjs"
);

const TEMPLATE_REQUEST_ERROR = path.join(
    __dirname,
    "../tmpl/request.error.ejs"
);
const OUTPUT_NODE_REQUEST_ERROR = path.join(opts.out, "request.error.node.cjs");
const OUTPUT_BROWSER_REQUEST_ERROR = path.join(
    opts.out,
    "request.error.browser.mjs"
);

const TEMPLATE_STANDALONE = path.join(__dirname, "../tmpl/standalone.ejs");
const OUTPUT_NODE_STANDALONE = path.join(opts.out, "standalone.node.cjs");
const OUTPUT_BROWSER_STANDALONE = path.join(opts.out, "standalone.browser.mjs");

const TEMPLATE_REPORTER = path.join(__dirname, "../tmpl/reporter.ejs");
const OUTPUT_NODE_REPORTER = path.join(opts.out, "reporter.node.cjs");
const OUTPUT_BROWSER_REPORTER = path.join(opts.out, "reporter.browser.mjs");
